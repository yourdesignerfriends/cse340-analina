...
-- Project Enhancement: I created the "favorites" table so users can mark vehicles as favorites.
-- It connects users with specific vehicles, ensures no duplicate favorites, and keeps the
-- database clean by removing related favorites automatically when a user or vehicle is deleted.
...
-- I created the "favorite_stats" view to see which vehicles are added as favorites
-- the most. This view does not show any client information, only the total number
-- of times each vehicle has been favorited. It helps admins quickly see which
-- models are the most popular without exposing personal data.
CREATE VIEW favorite_stats AS
SELECT 
  i.inv_id,
  i.inv_make,
  i.inv_model,
